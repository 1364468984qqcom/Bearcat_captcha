...
class Image_Processing(object):
    ...
    # 保存分类字典
    @classmethod
    def save_n_class(self, path_list: list, divide='_'):
        ...
        d = dict((index, name) for index, name in enumerate(dicts))
    ...
    # 根据文件名提取标签
    @classmethod
    def extraction_class_lable(self, path_list: list, divide='_'):
        dicts = self.extraction_dict(path_list)
        logger.debug(f'一共有{len(dicts)}类')
        paths = [os.path.splitext(os.path.split(i)[-1])[0] for i in path_list]
        path = [re.split(divide, i)[0] for i in paths]
        lable_list = [dicts.get(i) for i in path]
        return lable_list

    @classmethod
    def _one_hot_lable(self, string):
        ver = np.zeros(N_CLASS)
        ver[string] = 1
        return ver

    @classmethod
    def extraction_class_one_hot_lable(self, path_list: list, divide='_'):
        dicts = self.extraction_dict(path_list)
        logger.debug(f'一共有{len(dicts)}类')
        paths = [os.path.splitext(os.path.split(i)[-1])[0] for i in path_list]
        path = [re.split(divide, i)[0] for i in paths]
        lable_list = [self._one_hot_lable(dicts.get(i)) for i in path]
    ...
    # 单个字符转向量
    @classmethod
    def text2vector_ocr(self, string, class_dict):
        string = str(string)
        vector_ocr = []
        while True:
    ...
    # 提取文字的字典
    @classmethod
    def extraction_ocr_dict(self, path_list: list, divide='_'):
        paths = [os.path.splitext(os.path.split(i)[-1])[0] for i in path_list]
        path = [re.split(divide, i)[0] for i in paths]
        ocr_path = self.text2index(path)
        dicts = sorted(set(ocr_path))
        class_dict = dict((name, index) for index, name in enumerate(dicts))
        class_dict['_'] = len(class_dict)
        return class_dict

    # 保存分类字典
    @classmethod
    def save_ocr_class(self, path_list: list, divide='_'):
        paths = [os.path.splitext(os.path.split(i)[-1])[0] for i in path_list]
        path = [re.split(divide, i)[0] for i in paths]
        ocr_path = self.text2index(path)
        dicts = sorted(set(ocr_path))
        class_dict = dict((index, name) for index, name in enumerate(dicts))
        class_dict[len(class_dict)] = '_'
        with open('n_class.json', 'w', encoding='utf-8') as f:
    ...
    # 提取文字的标签
    @classmethod
    def extraction_ocr_lable(self, path_list: list, divide='_'):
        dicts = self.extraction_ocr_dict(path_list, divide=divide)
        logger.debug(f'一共有{len(dicts)}类')
        paths = [os.path.splitext(os.path.split(i)[-1])[0] for i in path_list]
        path = [re.split(divide, i)[0] for i in paths]
        lable_list = [self.text2vector_ocr(i, dicts) for i in path]
...
class WriteTFRecord(object):
    ...
    @classmethod
    def WriteTFRecord_ocr(self, TFRecord_path, datasets: list, lables: list, file_name='dataset.tfrecords'):
        num_count = len(datasets)
        lables_count = len(lables)
        if not os.path.exists(TFRecord_path):
            os.mkdir(TFRecord_path)
...
@tf.function
# 映射函数
def parse_function_ocr(exam_proto):
    features = {
        'image': tf.io.FixedLenFeature([], tf.string),
...
class Distinguish_image(object):
    ...
    def vector2text(self, vector):
        ...
        # recognition_rate = self.probability(vector)
        # logger.debug(len(char_pos))
        # logger.debug(len(vector))
    ...
    @classmethod
    def distinguish_image_n_class(self, model_path, jpg_path, suffix=True, divide='_'):
        model = tf.keras.models.load_model(model_path)
        forecast = model.predict(Image_Processing.load_image(jpg_path))
        lable_real = self.extraction_lable_name(jpg_path, suffix, divide)
